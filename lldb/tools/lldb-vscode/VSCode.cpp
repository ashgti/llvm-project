...
ExpressionContext VSCode::DetectExpressionContext(lldb::SBFrame &frame, std::string &text) {
  ...
  case ReplMode::Auto:
    ...
    lldb::SBValue value = frame.GetValueForVariablePath(text.data(), lldb::eDynamicDontRunTarget);
    ...
    if (value.GetError().Success()) {
      ...
      if (result.Succeeded() && !auto_repl_mode_collision_warning) {
        llvm::errs() << "Variable expression '" << text << "' is hiding an lldb command, prefix an expression with ` to ensure it runs as a lldb command.\n";
...
bool ReplModeRequestHandler::DoExecute(lldb::SBDebugger debugger,
  ...
  if (new_mode == "variable") {
    ...
    error_message.Printf("Invalid repl-mode '%s'. Expected one of 'variable', 'command' or 'auto'.\n",  new_mode.data());
