...
public:
  ...
  // FIXME: Migrate callers to typed RequestHandler and use the other form of
  // the `operator()` call.
...
class RequestHandler : public BaseRequestHandler {
  ...
  void operator()(const llvm::json::Object &request) const override {
    /* no-op while migrating to typed arguments and responses, handled by the
     * other overload. */
  }
  ...
  void operator()(const protocol::Request &request) const override {
    ...
    if (request.rawArguments &&
        !lldb_dap::protocol::fromJSON(*request.rawArguments, arguments, root)) {
    ...
    auto ResponseBody = Execute(arguments);
    if (!ResponseBody) {
      response.success = false;
      response.message = llvm::toString(ResponseBody.takeError());
    } else {
  ...
  /// Execute the request with the parsed arguments and returning the resulting
  /// reponse body.
  virtual llvm::Expected<Body> Execute(const Args &args) const = 0;
};
...
public:
  ...
  Execute(const protocol::CancelArguments &args) const override;
};
